...
`tools/watch_stellaris_live_autosaves.ps1` remains a manual development harness. PowerShell-based hidden startup capture was removed because Avast flags the HKCU Run persistence pattern; startup capture belongs in native SNC start-with-Windows behavior, not a hidden PowerShell launcher.
...
The same real-session loop now also emits entry-point-scoped post-play artifacts (`entry_point_analysis -> post_play_package -> decision_input_package -> candidate_decision_package`) plus readiness/evidence fields, so real-session validation can inspect the new branch-aware path without dropping the existing offline-spine baseline.
Those entry-point-scoped artifacts now also carry bounded compatible/later archived-evidence samples per loadable save so the conservative branch-aware path can preserve concrete historical references without inventing full branch timelines yet.
The same loop now also auto-resolves a bounded effective `campaign_id` and `empire_id` from single-campaign decision-input artifacts when the owner leaves them blank, and it auto-feeds the validated SNC DSL draft into `--run-offline-spine` when no explicit DSL path is provided. It still fails closed when post-play artifacts span multiple campaign keys or do not expose a bounded player identity.
...
`Strategic Nexus.exe --publish-snc-generated-overlay <staging_status.json> <active_overlay_dir> <publish_status.json> owner-approved [stellaris_running_override] [backup_root]` is the higher-level SNC publish gate: it requires explicit owner approval, treats staged LLM/DSL output as untrusted until status + manifest checks pass, creates a backup of any existing active generated overlay, and writes an audit JSON before reporting success.
SNC status snapshots and tray now expose the staged generated-overlay publish gate as owner-facing state (staging status, staged snapshot, active snapshot target, publish status output, backup root, manifest hash, rule count, owner approval, and `can_publish`). The active target is a separate generated-overlay snapshot directory, not the full mod root, so descriptor/localization/manual PoC files are not replaced by the generated publisher.
...
Steam Cloud userdata discovery prefers Steam's Windows registry install path so custom Steam locations on any drive are covered, then falls back to common `ProgramFiles*` paths.
...
The save-root contract is now explicitly provider-neutral: Steam is not required, and GOG/Galaxy, Microsoft Store/Xbox app, and Paradox Launcher/direct installs should be covered by Documents roots unless a distinct provider-specific cache is later verified.
...
`tools/run_real_session_v0_loop.ps1` now includes strict MP verify/import command hints in `real_session_v0_next_steps.md` and evidence JSON `command_hints` (`mp_strict_verify`, `mp_strict_import`), so host/client validation steps remain copyable from one run artifact.
When MP mismatch or identity-risk review becomes the top `next_action`, `tools/run_real_session_v0_loop.ps1` now also prefers a concrete strict-verify command hint source (`mp_export`, compare, or trend current package) instead of leaving owner/release-companion follow-up without an executable safety step.
`tools/run_real_session_v0_loop.ps1` next-steps brief now also includes MP snapshot routing/readiness plus host/client next-step text and package ZIP identity fields (`state`, `reason`, `sha256`, `bytes`) when MP export is enabled, so release-companion handoff can stay in the owner-facing brief without reopening evidence JSON.
...
Compare/trend/loop outputs now also expose structured aggregated MP mismatch drift fields (`*_mp_mismatch_warning_state_{previous,current,changed}`, `*_mp_mismatch_warning_reason_{previous,current,changed}`, `*_mp_mismatch_warning_code_{previous,current}`, `*_mp_mismatch_warning_codes_changed`), and loop evidence JSON mirrors these fields for one-file release-companion parsing of package-risk state transitions.
