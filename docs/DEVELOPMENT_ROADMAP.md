...
<!-- ROADMAP_PROGRESS_SNAPSHOT_START -->
# Roadmap Progress Snapshot

Auto-generated by `tools/dev_attention/analyze_roadmap_complexity.ps1` from roadmap item statuses and weighted complexity points.

| Metric | Value |
| --- | ---: |
| Weighted overall progress | 62% |
| Strict implementation complete | 47% |
| Implemented-or-better complexity | 47% |
| Verified-or-done complexity | 6% |
| Implemented-or-better items | 50% |
| Verified-or-done items | 8% |
| Roadmap items | 52 |
| Complexity points | 606 |
| Completed weighted points | 377 |
| Remaining weighted points | 229 |
| Remaining estimate | cca 4-10 tydnu podle tempa |
| Estimate confidence | stredni |

| Status | Items | Complexity points | Complexity share | Weighted completed points |
| --- | ---: | ---: | ---: | ---: |
| DONE | 0 | 0 | 0% | 0 |
| VERIFIED | 4 | 39 | 6% | 39 |
| IMPLEMENTED | 22 | 246 | 41% | 246 |
| IN_PROGRESS | 20 | 261 | 43% | 91.35 |
| BLOCKED | 1 | 13 | 2% | 0.65 |
| NOT_STARTED | 5 | 47 | 8% | 0 |
<!-- ROADMAP_PROGRESS_SNAPSHOT_END -->
...
The task-board mutators now invoke `sync_user_task_board_state.ps1` after writes on the default local files so the progress estimate, roadmap complexity, and queue refresh stay current without waiting for the next manual hygiene pass.
...
Owner-facing progress separates `implementation_complete_percent` (strict implemented/verified/done roadmap complexity) from `weighted_progress_percent` (forecast-style weighted progress that also credits in-progress or blocked slices). Use the strict implementation percent when Antonin asks how much implementation is actually done.
...
The canonical Task Board visual and interaction contract lives in `TASK_BOARD_UI_STYLE_GUIDE.md`; implementation should follow that document rather than screenshots or memory when the UI is updated.
...
Roadmap tracking:
This parent epic is not counted directly in project-completion percentages. Use the atomic slices below for implementation status.
...
* The companion presentation contract is documented in `SNC_UI_STYLE_GUIDE.md`; when implementation changes, keep that document and the roadmap in sync.

---

## 1A. Companion Lifecycle And Startup Contract

Status:
IMPLEMENTED

Goal:
Provide a native owner-facing companion process that can run continuously without becoming hidden persistence.

Required:

* native SNC tray process
* single-instance startup behavior
* close-window-keeps-running behavior
* explicit exit stops the companion
* optional Windows Startup-folder shortcut
* startup state visible in JSON, CLI, tray status, and owner brief
* crash restart policy documented and surfaced
* no hidden PowerShell launcher or HKCU Run persistence

Current progress:
`StrategicNexusCompanionTray.exe`, `tools/run_snc_tray.ps1`, startup-shortcut state, tray JSON status, and smoke coverage exist. This slice is implemented for v0; future work belongs in UX or reliability follow-up slices.

---

## 1B. Autosave Capture And Save-Root Discovery

Status:
IMPLEMENTED

Goal:
Preserve Stellaris autosaves before the game rotates them away, across local and cloud-backed save roots.

Required:

* provider-neutral save-root discovery
* Steam registry-aware discovery for custom Steam install locations
* Documents and Steam Cloud local-cache candidates
* dynamic campaign-folder rescanning
* stable-copy archive semantics for rotating `autosave*.sav` and `ironman.sav`
* hash-named archive copies for repeated active filenames
* live monitor status JSON
* owner-testable session capture command

Current progress:
`--discover-stellaris-save-roots`, `--archive-live-saves`, `--run-snc-session-capture`, and the native SNC live-monitor path exist and have been exercised against real save roots. This is implemented for v0 capture.

---

## 1C. Save Container Parser And Archive Summaries

Status:
IMPLEMENTED

Goal:
Read archived saves safely enough to produce bounded facts for downstream offline analysis.

Required:

* read-only ZIP container handling for `.sav`
* bounded `meta` and `gamestate` headline extraction
* explicit missing-field reporting
* archive verification before analysis
* archive metadata summary
* season delta ledger with parsed-headline facts when available
* empire brief builder from verified archive data
* ministry input builder from archive data

Current progress:
`--verify-autosave-archive`, `--summarize-autosave-archive`, `--parse-stellaris-save`, `--build-season-delta-ledger`, `--build-empire-brief-from-archive`, and `--build-ministry-input-from-archive` exist. This slice is implemented for bounded v0 headline parsing and archive summaries.
...
* loadable save entry-point discovery
* branch/reload-aware autosave partitioning
* compatible archived evidence samples per entry point
* later archived evidence samples kept separate
* post-play package output
* decision-input package output
* candidate-decision package output
* fail-closed handling for ambiguous campaign or player identity
...
## 1E. Offline Spine And Validated DSL Overlay Compiler

Status:
IMPLEMENTED

Goal:
Prove the first end-to-end offline path from verified archive evidence to a validated generated overlay.

Required:

* deterministic offline spine command
* explicit validated DSL input
* DSL parser and validator
* generated overlay compiler
* generated manifest with hashes and byte counts
* fail-closed overlay verifier
* tests for successful staging and unsafe DSL rejection
* no raw LLM script accepted as trusted mod output

Current progress:
`--run-offline-spine`, `--compile-generated-overlay`, `--verify-generated-overlay`, generated manifest output, and generated-overlay contract tests exist. This slice is implemented for v0 explicit-DSL offline overlay generation.

---

## 1F. Generated Overlay Publish Gate And Status Center

Status:
IN_PROGRESS

Goal:
Promote generated overlays into the active mod snapshot only through owner-visible, fail-closed checks.

Required:

* staged overlay verification before publish
* refusal to publish while Stellaris is running
* owner approval gate for SNC publish
* backup of existing active generated overlay
* complete replacement snapshot publishing
* publish status JSON
* active/staged/generated-overlay identity in Status Center
* gameplay acceptance status surfaced to owner
...
## 1G. Local LLM Model Manager

Status:
IN_PROGRESS

Goal:
Let the companion manage a user-selected supported local model without distributing model weights or trusting raw model output.

Required:

* supported model catalog
* no LLM weights distributed with the mod
* reduced-mode warning when no supported model is selected
* local runtime readiness check
* explicit license acceptance before download
* user-selected model state JSON
* fail-closed unknown-model handling
* validator-owned promotion from model output to DSL/generated overlay

Current progress:
SNC status exposes local LLM readiness fields and `--prepare-local-llm-model` can check/pull an allowlisted Ollama model after explicit license acceptance. Companion and tray surfaces now also expose `local_llm_install_guidance` on current head so reduced-mode states can tell the owner what to install or prepare next without raw JSON spelunking, and tray JSON now also mirrors `local_llm_prepare_command_hint` so the prepare command can be copied without scraping the visible summary text. The owner-facing status surfaces now also expose the curated supported-model catalog summary so the model manager can show what is allowed before selection. Remaining work is owner-facing SNC Model Manager UI and first validated inference-to-DSL handoff.
The owner-facing next-steps brief now also mirrors the local LLM model-manager summary, model-state path, and install guidance so the owner can open the manager flow from the durable brief without reopening the raw status snapshot.
...
## 1H. Reactive Policy Pack Dispatch

Status:
IN_PROGRESS

Goal:
Move the project toward legal near-realtime behavior by precompiling validated event-family branches into the generated overlay.

Required:

* allowlisted event-family DSL branches
* rejection of raw `on_*` event names from untrusted DSL
* generated manifest event-family coverage
* handwritten base-kernel dispatch touchpoints
* monthly strategy tick dispatch path
* war-started dispatch path
* country-attacked dispatch path
* owner-ready monthly reactive test contract
* fail-closed readiness surfacing in SNC and real-session loop outputs
...
## 1I. SNC Owner UI And Presentation

Status:
IN_PROGRESS

Goal:
Make SNC feel like the same owner-facing product family as the Task Board instead of a raw diagnostic window.

Required:

* Czech owner-facing labels
* TB-like dark/gold/teal visual language
* custom title/header controls
* branded transparent SNC icon
* read-only status fields with wrapping
* styled scroll behavior
* resize/repaint correctness
...
* first-run explanation for early SNC startup
* routine staging decisions automated when safe
* owner prompts only for meaningful approvals
* local support/crash report preview before sending
* Status Center actions for archive, model, publish, and MP package state
* reduced-mode guidance when required dependencies are missing
* clear recovery paths for blocked or degraded states

Current progress:
Underlying status and command surfaces exist, but the full low-friction release companion workflow is not implemented yet. The first-run startup rationale is now an explicit release-companion contract: SNC snapshot JSON, tray JSON, summary text, and next-steps brief all explain that early SNC startup preserves more autosave history before Stellaris rotates older saves away, while start-with-Windows remains optional and default-disabled. The visible status dashboard now also exposes a generated-overlay publish action that routes through the existing owner approval confirmation and Stellaris-closed publish gate instead of bypassing validation. The dashboard also exposes a fail-closed next-action path button that opens the current `next_action_path` artifact only when that file or directory exists. Degraded or attention-needed memory recovery now becomes a stable top-level `review_memory_recovery_status` next action when no higher-priority MP/crash/publish action applies, and its path points at entry-point analysis evidence for direct dashboard follow-up.
The support report preview helper now also emits a direct open-command hint for the generated preview file, and the v0 pipeline regression covers that preview-output contract so the manual review step stays one click away.
...
The scripted PoC receiver now projects supported `military_posture` and `research_bias` branches into visible gameplay-neutral country modifiers, so real-session owners can confirm which bounded v0 preference reached the mod layer without relying on raw JSON alone.
...
## 2A. Strategy Dimension Validation Contract

Status:
IMPLEMENTED

Goal:
Keep v0 strategy dimensions small, bounded, and safe to route into generated overlays.

Required:

* `military_posture` allowlist
* `research_bias` allowlist
* rejection of legacy `doctrine_id`
* bridge-core reuse for v0 cabinet enum validation
* bounded malformed-input failure tests
* audit-visible final payload

Current progress:
Bridge core and v0 pipeline tests cover the two v0 domains, malformed input rejection, enum reuse, and final payload auditing. This slice is implemented for v0.

---

## 2B. Deterministic V0 Decision Pipeline

Status:
IMPLEMENTED

Goal:
Transform bounded ministry input into a deterministic validated decision payload before any LLM path is trusted.
...
* ministry proposal `Y`
* clerk brief `U`
* final `strategy_dimensions` payload
* replay-protection sequence state
* atomic output writes
* audit output that does not invalidate otherwise valid decisions
* regression tests for fallback and cleanup paths

Current progress:
The v0 deterministic pipeline, audit record, priority score, atomic writes, failure cleanup tests, and local CI suite exist. This slice is implemented for the non-LLM deterministic v0 path.

---

## 2C. Mod-Side Observable Strategy Markers

Status:
IN_PROGRESS

Goal:
Make generated v0 strategic choices observable in game without introducing hidden buffs or cheating behavior.

Required:

* generated flags or markers for supported strategy dimensions
* clearing stale generated preference flags per affected domain
* scripted mod receiver path for generated markers
* visible gameplay-neutral owner marker
* monthly-pulse marker refresh path
* no hidden resource, combat, or AI bonuses
...
## 2D. Generated Overlay Gameplay Acceptance Gate

Status:
IN_PROGRESS

Goal:
Prove generated overlays produce the intended bounded gameplay effect before they are treated as gameplay-ready.

Required:

* documented owner-facing acceptance plan
* repeatable generated overlay acceptance harness
* evidence JSON for v0 cases
* pass/fail surfaced in SNC Status Center
* real-session owner validation path
* fail-closed state when acceptance is absent or stale

Current progress:
`GENERATED_OVERLAY_GAMEPLAY_ACCEPTANCE_PLAN.md`, `tools/run_generated_overlay_gameplay_acceptance.ps1`, and `generated_overlay_gameplay_acceptance_v0.json` exist. Remaining work is real-session validation against a loaded Stellaris mod surface, not just generated-file and harness-level checks.
...
The post-play package parser now has an explicit regression that rejects future `schema_version` values, so entry-point-scoped downstream handoff fails closed on incompatible artifact drift instead of silently ingesting unknown shapes.

---

## 3C. Identity-Scoped Ministry Context Contract

Status:
IMPLEMENTED

Goal:
Prevent global or identity-free context from being treated as trusted empire-specific input.
...
* fail-closed missing identity handling
* two-empire same-campaign regression

Current progress:
The v0 ministry input context carries the required identity fields, fails closed when identity is missing, and has two-empire regression coverage. This slice is implemented for v0 empire isolation.

---

## 3D. Entry-Point Identity Handoff

Status:
IN_PROGRESS

Goal:
Keep campaign and player identity stable when the user can load multiple saves or roll back to older entry points.

Required:

* campaign identity in post-play package
* empire identity in decision-input package
* safe single-campaign auto-resolution
* fail-closed multi-campaign ambiguity
* loadable-save entry-point identity fields
* downstream parser rejection of unsupported schema versions
...
The same real-session compare/trend/loop evidence now also forwards explicit entry-point campaign_key and empire_name identity fields, so the loadable-save identity pair is visible alongside the existing analysis path and readiness metadata.
...
* per-empire parser field availability map
* source-quality per field group
* explicit missing-field reasons
* no gameplay-affecting output from unknown evidence
...
Current progress:
The first observer-target profile contract exists as a summary-only builder. `ObserverTargetProfileBuilder` now accepts a validated observer brief plus a target empire id and confidence, records bounded evidence references, emits a deterministic confidence-scored target memory summary, derives a conservative predictive-dimensions summary from validated evidence keywords, populates bounded relationship-delta summaries, exposes an explicit observer/target/diplomacy/war/subject/federation/border/intel field-availability map, and now also exposes a fail-closed target-specific rule-candidate validation scaffold that keeps candidate domains visible without enabling gameplay output. It intentionally keeps target-specific rule candidates empty until later validation and gameplay-safe generation exist.
`ObserverTargetProfileBuilder` now also exposes compact field-availability counts and a summary contract mirroring the integrated empire-state availability summary, so downstream consumers do not need the full array just to see which evidence groups are present or missing.
Current-head verification confirms `dist/observer_target_profile_builder_test.exe` passes with the compact count/summary contract visible in the profile JSON.
...
The profile contract still needs actual target-specific interpretation and generated overlay rule candidates before it can influence gameplay.
...
Current progress:
Save entry-point analysis now distinguishes currently loadable campaign saves from historical archived autosaves when metadata allows it, and the bounded post-play package carries branch ambiguity explicitly instead of collapsing divergent history into one linear truth.
The first v0 post-play handoff now preserves compatible archived evidence samples separately from later archived evidence so future entry-point-scoped memory/bootstrap work can stay portable and fail closed on timeline drift.

---

## 4A. Entry-Point Archive Timeline Partitioning

Status:
IMPLEMENTED

Goal:
Separate currently loadable saves from archived historical evidence so rollback/reload paths do not collapse into one false timeline.

Required:

* loadable save vs archived autosave distinction
* branch ambiguity surfaced explicitly
* compatible evidence samples
* later evidence samples separated from compatible evidence
* fail-closed handling when timeline provenance is incomplete

Current progress:
Save entry-point analysis and post-play handoff preserve loadable-vs-historical distinction and compatible/later evidence samples. This slice is implemented for v0 partitioning.

---

## 4B. Branch-Aware Memory Reconstruction

Status:
IN_PROGRESS

Goal:
Reconstruct campaign memory only from evidence compatible with the entry point the user can actually load next.

Required:

* branch-aware autosave chain selection
* rollback/reload detection
* per-entry-point memory input selection
* safe uncertainty when branch evidence conflicts
...
* latest available loadable saves as recovery anchors
* explicit confidence downgrade when autosave history is missing
* durable campaign memory merge
* no invented timeline gaps
...
## 5A. Host-Only MP Authority Contract

Status:
IMPLEMENTED

Goal:
Keep multiplayer Strategic Nexus decisions host-owned and deterministic for clients.

Required:

* clients never process LLM payloads independently
* generated gameplay-affecting overlay package is host-owned
* package identity is deterministic
* local client state cannot diverge gameplay-affecting output
* architecture documented in MP orchestrator

Current progress:
The host-authoritative MP package direction is documented and the package workflow is host-owned. This slice is implemented as an architecture and v0 package contract.

---

## 5B. MP Byte-Identity Verification

Status:
IMPLEMENTED

Goal:
Verify host and client generated gameplay-affecting files are byte-identical before MP use.

Required:

* generated overlay manifest
* checksum-sensitive file list
* deterministic hashes
* verifier failure on drift
* warning codes for mismatch conditions

Current progress:
The MP package verifier fails closed on file drift, unexpected files, and identity/version/hash mismatches. This slice is implemented for v0 byte-identity verification.

---

## 5C. Real MP Desync And Fallback Validation

Status:
VERIFIED

Goal:
Prove the package workflow behaves safely in a real or faithful multiplayer validation scenario.

Required:

* real or faithful MP validation session
* desync-risk observation
* fallback path when a client has the wrong overlay
* host/client recovery instructions tested
...
This slice is verified on current head for the faithful fallback contract; the full owner-run real MP desync session remains a separate owner-facing evidence step.
...
## 6A. Safe Integration Boundary

Status:
IMPLEMENTED

Goal:
Keep Strategic Nexus inside supported mod scripting, generated files, local archives, and bounded JSON payloads.

Required:

* no running-process patching
* no proprietary executable analysis
* no arbitrary command execution from model output
* no save editing
* generated overlay files as the gameplay delivery path
* bridge-core validation before mod-facing output
* manual development harnesses clearly separated from production integration

Current progress:
The project direction is documented and implemented around generated overlays, bridge-core validation, and local archive analysis. The forbidden integration paths are explicitly out of scope. This slice is implemented as a project safety boundary.

---

## 6B. Save Campaign Inventory And Root Discovery

Status:
IMPLEMENTED

Goal:
Discover locally playable Stellaris campaign saves without mutating user data.

Required:

* read-only campaign directory scan
* loose `.sav` support
* deterministic inventory JSON
* anchor save fingerprint
* provider-neutral save-root discovery
* Steam registry-aware Steam Cloud cache discovery
* root-diff harness

Current progress:
`--scan-save-campaigns`, `--discover-stellaris-save-roots`, and `--diff-save-campaigns` exist. The scan includes anchor fingerprints and root diffs. This slice is implemented for v0 inventory.

---

## 6C. Active Campaign Library Planning And Compile

Status:
IMPLEMENTED

Goal:
Build a bounded active generated campaign library from campaigns that are actually playable locally.

Required:

* campaign library plan JSON
* max-campaign limit
* skip reasons for overflow
* compile only matching campaign DSL rules
* marker-guarded rulesets
* fail-closed missing save root
* fail-closed non-empty output directory
* complete replacement snapshot semantics

Current progress:
`--plan-campaign-library` and `--compile-campaign-library-overlay` exist, enforce bounds, and fail closed on unsafe output or unavailable save roots. This slice is implemented for bounded v0 active-library planning.

---

## 6D. Campaign Library Continuity Signals

Status:
IMPLEMENTED

Goal:
Keep local campaign-library continuity visible when saves are renamed, restored, removed, or truncated by limits.

Required:

* stable rename detection from anchor fingerprint
* restored-continuity detection across source kind changes
* ambiguous rename fallback
* limit-reached signal
* skipped-count signal
* SNC/tray/status visibility
* real-session loop forwarding
* compare/trend drift visibility

Current progress:
Inventory diff, campaign-library plans, SNC status surfaces, real-session loop output, and compare/trend outputs carry rename/restored/limit/sidecar signals. This slice is implemented for v0 visibility.
...
* final campaign identity from save contents when available
* stable local folder aliases
* fallback identity when evidence is weak
* owner-visible ambiguity state
* rule generation blocked or conservative when identity is unsafe
...
* pinned campaign state
* owner-facing pin/unpin action
* bounded active-library size handling with pins
* clear warning when a pinned campaign is not locally playable
...
## 6G. Generated Campaign Library Maintenance Epic
...
## 9A. Zero-History Bootstrap Personality Contract

Status:
IN_PROGRESS

Goal:
Give unknown or new campaigns conservative, varied personality defaults without binding personality to a fixed slot number forever.

Required:

* zero-history bootstrap marker
* deterministic but rotating bootstrap metadata
* source-quality surfaced in generated manifest
* conservative defaults when no history exists
* variation across policy-pack updates
* history-backed continuity overrides bootstrap variation once evidence exists
...
Current progress:
  ...
  StrategicWorker and request-file parsing now also accept an optional `campaign_id` plus `personality_profile_root` pair so validated profiles can be reused in the prompt-output path when the caller knows the campaign, and the v0 pipeline regression now covers that reuse path on current head.
...
* trust/distrust
* trauma memory
* consistency checks against capability and evidence
* downgrade/reject path for contradictory recommendations

Current progress:
  StrategicWorker now refines doctrine choices through personality and capability checks for the dominant empire, and the LLM prompt surfaces the resulting personality bias when that focus empire is available. The regression coverage now also exercises a capability-only opportunistic-expansion downgrade path plus multiple explicit contradiction reject paths, including the hegemony-aware reject path added on current head, the trauma/low-trust balance_against_hegemon reject path now covered without confirmed hegemonic pressure, and the trauma-only rising-pressure balance_against_hegemon reject path added on current head, while the alignment note already flows through prompt, JSON, and worker stdout. Current-head verification now also keeps the same alignment note visible in real-session compare, trend, and loop evidence output via the post-play doctrine alignment note field. Remaining work is broader contradiction coverage, richer downgrade/reject paths, and any additional owner-facing surface that needs the note made even more prominent.
Current-head verification now also covers a rising-pressure balance_against_hegemon reject path for non-traumatized low-trust empires, so coalition balancing stays fail-closed once distrust remains the deciding signal even as pressure rises.
  ...
  Current-head verification now also covers a calm-hegemon consolidate reject path for weak empires without explicit fear, keeping the immediate-defense fallback explicit even when the bias reads as cautious consolidation.
  Current-head verification now also covers a hegemony-aware consolidate reject path for capable but fearful empires, which keeps fragile consolidation fail-closed even when capability is otherwise adequate.
  ...
  Current-head verification now also covers a rising-pressure consolidate reject path, which keeps weak fearful empires from treating clearly rising pressure as a reason to keep fragile consolidation instead of switching to immediate defense.
  ...
  Current-head verification now also covers a moderate-pressure opportunistic expansion reject path, which keeps weak fearful empires from treating moderate pressure as a reason to stay opportunistic instead of switching to immediate defense.
  Current-head verification now also covers a rising-pressure opportunistic expansion reject path, which keeps weak fearful empires from treating clearly rising pressure as a reason to stay opportunistic instead of switching to immediate defense.
  ...
  Current-head verification now also covers a low-honor, high-opportunism balance_against_hegemon reject path under confirmed hegemonic pressure, so unreliable coalition balancing fails closed even when the empire is otherwise capable.
  Current-run verification now also covers a low-honor, high-opportunism opportunistic-expansion reject path under confirmed hegemonic pressure, so reckless expansion stays fail-closed even when capability is otherwise adequate.
  ...
  Current-run verification now also covers the generic fear-only opportunistic-expansion fallback on a capable empire, so the non-special-case downgrade path stays covered even when capability is otherwise adequate.
  Current-head verification now also covers a capable empire under elevated pressure rejecting opportunistic expansion, so the doctrine gate stays fail-closed once pressure has climbed beyond the low-risk window even without a confirmed hegemon.
...
Goal:
Let civilizations build distinct strategic culture and reputation interpretations over time.

Required:

* strategic culture fields
* private self-image vs perceived reputation split
* observer/audience-aware interpretation
* memory-backed reputation changes
* conservative output when evidence is weak

Current progress:
This is future strategic intelligence work and should wait for parser/evidence contracts.
...
Current progress:
The summary-only integrated empire-state contract now also surfaces a bounded capability constraints summary from the available fleet/economic/technology/influence signals while keeping infrastructure/logistics/transition-cost evidence explicitly missing. The new summary stays fail-closed and does not add gameplay-affecting reasoning yet.
...
The post-play package parser now rejects unsupported future `schema_version` values before downstream entry-point handoff, so incompatible artifact drift fails closed instead of being silently accepted.
Compatibility and migration layers are not fully implemented yet.

---

## 12A. Versioned Artifact Fail-Closed Contract

Status:
IMPLEMENTED

Goal:
Reject incompatible future artifacts instead of silently accepting unknown shapes.

Required:

* `schema_version` required on bridge/core artifacts
* unsupported future schema rejection
* post-play package parser compatibility check
* tests for incompatible schema failure
* no silent unknown-shape ingestion

Current progress:
Bridge core requires `schema_version`, and the post-play package parser rejects unsupported future schema versions with regression coverage. This slice is implemented for v0 fail-closed versioning.
...
* generated overlay validator coverage
* explicit capability/event-family metadata
...
## 12C. Backward Migration Rules

Status:
IN_PROGRESS

Goal:
Keep long-lived campaign memory and generated artifacts usable across future schema migrations.

Required:

* migration registry
* backward-compatible readers where safe
* explicit partial compatibility state
* owner-visible migration/degraded-state warnings
* tests for old artifact migration
...
The SNC DSL draft package builder now also carries source candidate schema version plus compatibility state/note metadata through the audit JSON and companion/tray status surfaces, so the legacy provenance stays visible after the decision package becomes a draft.
...
2026-06-13 verification reran `dist/campaign_library_planner_test.exe`, `dist/strategic_nexus_companion_test.exe`, and `tools/smoke_snc_tray.cmd`; all passed and the same campaign library plan plus pinned-manifest degraded-state warnings stayed visible on current head.
2026-06-13 repeat verification reran `dist/campaign_library_planner_test.exe`, `dist/strategic_nexus_companion_test.exe`, and `tools/smoke_snc_tray.cmd`; all passed again and the same campaign library plan plus pinned-manifest degraded-state warnings stayed visible on current head.
2026-06-13 current-run verification reran `dist/campaign_library_planner_test.exe`, `dist/strategic_nexus_companion_test.exe`, and `tools/smoke_snc_tray.cmd`; all passed and the same campaign library plan plus pinned-manifest degraded-state warnings stayed visible on current head.
Current-head verification now also confirms the companion and tray status surfaces expose the post-play package entry-point-analysis source schema version plus compatibility state/note from the post-play package artifact, keeping the provenance visible alongside the existing post-play identity and personality notes.
...
`Strategic Nexus.exe --snc-status-snapshot` stable stdout now also emits `snc_window_close_behavior`, `snc_explicit_exit_behavior`, and `snc_crash_restart_policy`, so CLI consumers can read the bounded SNC lifecycle skeleton directly from stdout instead of opening the snapshot JSON lifecycle object.
...
Owner-controlled start-with-Windows for SNC is implemented as a normal Windows Startup-folder shortcut to the native tray executable. Avoid hidden PowerShell launchers or HKCU Run persistence for this path.
Generated overlay DSL now supports explicit `source_quality`, deterministic bootstrap rotation metadata, and allowlisted reactive event-family branches; the generated manifest now exposes reactive-policy capability/event-family coverage plus per-campaign source-quality/bootstrap metadata while rejecting raw `on_*`-style event family names.
The handwritten reactive dispatcher path is now wired for the v0 allowlisted families: the generated overlay compiler emits stable dispatch effects, the handwritten monthly policy kernel calls `strategic_nexus_generated_monthly_strategy_tick_dispatch`, and low-frequency proxy events route `war_started` and `country_attacked` through generated dispatch effects in the PoC mod surface. `tools/run_v0_pipeline_tests.ps1` asserts both the generated reactive dispatcher text and the base kernel `on_actions`/event wiring, so the remaining reactive-policy gap is real-session owner validation rather than another base dispatcher implementation chunk.
...
MP package export/verify/import plus SNC/tray surfaces now also expose fail-closed bootstrap provenance (`provenance_state`, `source_quality*`, `bootstrap_campaign_count`) so host-visible handoff artifacts can distinguish history-backed overlays from `zero_history_bootstrap` fallback without reopening the generated overlay manifest manually.
...
Compare/trend/loop outputs now also expose structured next-action path drift fields (`*_next_action_path_{current,previous,changed}`), and loop evidence JSON mirrors them in auto compare/trend blocks so release-companion can follow the exact staged artifact path, not just the textual next-step hint.
...
Add SNC friend MP sync outbox plan gate.

  `Strategic Nexus.exe --plan-snc-friend-mp-sync-outbox` now adds a local fail-closed send/transport plan for SNC friend MP sync envelopes. Valid metadata plus an encrypted payload still reports `transport_enabled=false` and `send_allowed=false` until signed/encrypted transport exists, while invalid envelopes, missing payloads, running Stellaris, and disabled friend auto-sync each produce explicit blocked states.

Next worker-ready slice:

Surface SNC friend MP sync outbox-plan template.

  The next slice should include:

* companion/tray status exposes a stable `--plan-snc-friend-mp-sync-outbox` command template
* the visible SNC dashboard can copy the template with no-send/no-upload/no-staging wording
* companion, tray smoke, and v0 pipeline tests verify the status/dashboard contract

  ---

## 15A. MP Package Export And Verification Contract

Status:
IMPLEMENTED

Goal:
Produce and verify deterministic host-owned generated overlay packages for multiplayer participants.

Required:

* package manifest
* campaign id, overlay version, game version, and mod version metadata
* checksum-sensitive generated gameplay file list
* local-only diagnostic manifest entry
* deterministic file hashes and byte counts
* fail-closed unexpected-file detection
* verifier warning codes
* host/client readiness status text

Current progress:
The MP package exporter/verifier exists, writes the manifest, validates file identity, emits structured warning codes, and produces host/client readiness text. This slice is implemented for v0 package verification.

---

## 15B. MP Import And Identity Mismatch Safety

Status:
IMPLEMENTED

Goal:
Let a client or next host import a package only through fail-closed identity/version checks.

Required:

* import command
* expected campaign id check
* expected overlay version check
* expected game version check
* expected mod version check
* expected manifest hash check
* structured mismatch warning codes
* strict verify/import command variants
* stable CLI fields for release companion consumption

Current progress:
`--verify-mp-overlay-package` and `--import-mp-overlay-package` expose structured mismatch warnings, strict commands, readiness fields, warning codes, and host/client next-step fields. This slice is implemented for v0 import safety.

---

## 15C. MP Status Center And SNC Visibility

Status:
IMPLEMENTED

Goal:
Expose multiplayer package readiness and risk in SNC/Status Center without requiring manual JSON parsing.

Required:

* SNC snapshot MP package fields
* Status Center summary readiness
* warning count and warning-code visibility
* host readiness field
* client readiness gate field
* verify/import command fields
* identity mismatch alert
* top-level next-action priority for identity risk

Current progress:
SNC snapshot JSON, `--snc-status-snapshot`, tray status, and Status Center summary expose MP package readiness, warning codes, identity mismatch state, verify/import commands, and top-level next actions. This slice is implemented for v0 owner visibility.

---

## 15D. MP Real-Session Evidence And Drift Contracts

Status:
IMPLEMENTED

Goal:
Carry MP package state through real-session loop, compare, and trend artifacts so release companion tooling can consume one stable evidence stream.

Required:

* real-session loop MP export fields
* evidence JSON MP export block
* compare output for package hash/readiness/warnings
* trend output for package hash/readiness/warnings
* next-action drift fields
* command-hint drift fields
* status-center drift fields
* warning arrays serialized as strings

Current progress:
Real-session loop, compare, trend, and evidence JSON outputs carry MP readiness, command hints, warning-code sets, identity risk, Status Center drift, next-action drift, and run metadata. This slice is implemented for v0 evidence plumbing.

---

## 15E. MP ZIP Handoff And Host Rotation Continuity

Status:
VERIFIED

Goal:
Support low-friction host handoff and host rotation without losing package identity or previous-host continuity.

Required:

* deterministic MP package ZIP artifact
* ZIP path, byte count, and SHA-256 surfaced in outputs
* previous-host availability signal
* degraded previous-host continuity warning
* manual save recovery hint
* compare/trend handoff drift visibility
* owner-facing brief lines for host/client readiness

Current progress:
The ZIP artifact, SHA/byte fields, previous-host continuity fields, degraded handoff recommendation, and owner-facing brief lines exist. `dist/strategic_nexus_companion_test.exe` and `tools/run_v0_pipeline_tests.cmd` pass on current head, so this continuity slice is verified. Remaining work is release-companion UX and real MP season validation.

---

## 15F. Human-Control Guarded MP Rules

Status:
IMPLEMENTED

Goal:
Generate campaign rules for all detected MP empires while ensuring rules never apply to an empire currently controlled by a human player.

Required:

* save-persisted campaign/empire/human-control markers
* deterministic bootstrap personalities for zero-history MP campaigns
* generated rules for all detected empires
* human-controlled empire activation guard
* single-player-founded player empire do-not-apply fallback
* host-owned byte-identical output for clients
* tests for human-control guard behavior

Current progress:
The compiler and package verifier already enforce the human-control guard with runtime `is_ai = yes` gating, the package manifest and verifier expose `human_control_guard_state`, and the companion status surfaces keep the guard visible to the owner. Current v0 contract tests cover the guard behavior on current HEAD.

---

## 15G. Release Companion MP Package UX
...
* strict verify/import buttons or command hints
* mismatch warning presentation
...
Current head now also has a first bounded outbox staging helper that can copy a verified friend MP sync envelope and encrypted payload into the selected transport adapter folder with a deterministic manifest, but the full automatic adapter chain and companion-facing surfacing still remain for follow-up.
...
The v0 pipeline regression now also covers the fail-closed `not_configured` and `needs_attention` transport-adapter probe states for missing trust-store and missing-folder inputs, so the adapter boundary stays explicit before signed/encrypted transport exists.
...
The v0 pipeline regression now also covers the inbox staging helper on current head, so the selected-folder receive side stays explicitly verified alongside the existing outbox staging path.
