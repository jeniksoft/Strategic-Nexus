...
Add SNC friend MP sync inbox plan gate.

  `Strategic Nexus.exe --plan-snc-friend-mp-sync-inbox` now exposes a fail-closed local inbox/staging plan for received friend MP sync envelopes. Invalid envelopes, missing encrypted payloads, running Stellaris, and disabled friend auto-sync each produce explicit states, and even a present payload with enabled auto-sync still reports `package_staging_allowed=false` until signed/encrypted transport, decrypt/verify, and guarded staging exist.
