...
| Completed weighted points | 292 |
| Remaining weighted points | 301 |
...
| IN_PROGRESS | 14 | 161 | 27% | 56.35 |
| BLOCKED | 1 | 13 | 2% | 0.65 |
| NOT_STARTED | 16 | 184 | 31% | 0 |
...
Current progress:
The compiler supports allowlisted reactive families and tests cover `monthly_strategy_tick`, `war_started`, and `country_attacked`. The monthly handwritten bridge exists. Remaining work is the base `on_action` dispatcher wiring for all approved branches and real-session owner validation.
...
The first handwritten reactive monthly bridge is now wired: the generated overlay compiler emits a stable `strategic_nexus_generated_monthly_strategy_tick_dispatch` effect, the handwritten monthly policy kernel calls that touchpoint, and `tools/run_v0_pipeline_tests.ps1` now proves the generated overlay/base-kernel monthly branch path with both empty and populated event-family fixtures. The compiler tests currently cover the allowlisted reactive families `monthly_strategy_tick`, `war_started`, and `country_attacked`; the next worker-ready slice is the remaining base `on_action` dispatcher wiring that routes those branches through the generated overlay path.
