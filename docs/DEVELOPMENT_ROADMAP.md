...
Surface SNC friend MP sync apply-gate input in templates.

  Companion and tray friend MP sync command templates now include the optional `[stellaris_running:true|false]` verify argument, and the visible dashboard copy text tells the owner to set it true while the game is active. This keeps the fail-closed apply gate visible from release-companion surfaces before any automatic signed/encrypted transport or package staging exists.
