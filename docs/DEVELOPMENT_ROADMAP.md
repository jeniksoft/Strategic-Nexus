...
Next architecture-aligned worker step: add archive-fed v0 acceptance assertions that verify the hinted turn-context values (including pressure) propagate through `--build-ministry-input-from-archive` and `--v0-pipeline-from-archive` without weakening fail-closed behavior.
