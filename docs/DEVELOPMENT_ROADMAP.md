...
`ArchiveMinistryInputBuilder` now threads parsed-headline war-state into ministry turn-context defaults from `headline_active_war_count`, adds explicit hint confidence metadata, and fails closed with explicit uncertainty when the war-count hint is missing or invalid.
Next architecture-aligned worker step: thread additional parsed-headline fields into bounded ministry turn-context defaults (for example date/year pressure hints) with explicit confidence and fail-closed fallback behavior.
