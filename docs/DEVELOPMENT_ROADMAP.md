...
Next architecture-aligned worker step: extend the same bounded parsed-save-summary feed into `--build-ministry-input-from-archive` and `--v0-pipeline-from-archive` so ministry context moves beyond pure metadata placeholders.
