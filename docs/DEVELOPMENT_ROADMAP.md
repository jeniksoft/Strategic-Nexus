...
Surface SNC friend MP sync inbox-plan template.

  Companion and tray status now expose `friend_mp_sync_inbox_plan_command_template`, and the visible SNC dashboard has an `SNC inbox` action that copies the fail-closed inbox-plan command. The copy text is explicit that this command does not download, decrypt, stage, or apply gameplay files.

Next worker-ready slice:

Continue Release Companion MP Package UX from the next bounded signed/encrypted transport, decrypt/verify, or guarded staging contract.
...
* prefer fail-closed transport/decrypt/staging contracts over cosmetic wording
