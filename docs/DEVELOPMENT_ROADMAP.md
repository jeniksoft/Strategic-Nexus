...
| Roadmap items | 19 |
| Complexity points | 247 |
| Completed weighted points | 59.1 |
| Remaining weighted points | 188 |
...
| IN_PROGRESS | 10 | 145 | 59% | 50.75 |
| BLOCKED | 1 | 13 | 5% | 0.65 |
| NOT_STARTED | 7 | 78 | 32% | 0 |
...
Next worker-ready slice:
 ...
 Surface the full SNC lifecycle skeleton in tray status JSON.

  The next slice should include:

* expose `window_close_behavior`, `explicit_exit_behavior`, and `crash_restart_policy` in `snc_tray_status.json` so release-companion or owner tooling can read the full lifecycle contract from the tray surface too
* keep the field names and values aligned with the existing snapshot JSON lifecycle object and the new CLI lifecycle stdout fields
* add regression coverage so the lifecycle tray JSON contract does not silently drift or disappear
