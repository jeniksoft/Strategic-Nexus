...
Next architecture-aligned worker step: thread selected parsed-headline fields into ministry turn-context defaults (for example bounded war-state hints) with explicit confidence and fail-closed fallback behavior.
