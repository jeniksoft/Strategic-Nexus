...
* A capture session is not necessarily a single linear branch even inside one campaign. If the player reloads an older save and plays forward again, captured autosaves can contain abandoned and current branches.
...
* After `stellaris.exe` exits, SNC can close the capture window, verify the archive, scan current active save roots for loadable entry points, analyze archived autosaves per source campaign folder/branch where possible, and generate/stage new mod rules for the next launch.
* The currently loadable saves are possible next-session entry points. The archive is historical evidence. Do not treat every captured autosave as a future entry point unless it is still loadable or intentionally restored/exported.
* See [SAVE_ENTRY_POINT_AND_BRANCH_RULES.md](SAVE_ENTRY_POINT_AND_BRANCH_RULES.md) for the canonical entry-point and branch model.
