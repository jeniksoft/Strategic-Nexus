    ...
    while ((Get-Date) -lt $deadline) {
        if (Test-Path -LiteralPath $statusPath) {
            ...
            if (
                ...
                if ([string]$json.friend_mp_sync_transport_adapter_state -ne "ready_disabled") {
                    throw "SNC tray status JSON did not expose ready_disabled friend_mp_sync_transport_adapter_state."
                }
                if ([string]$json.friend_mp_sync_transport_adapter_reason -notlike "*configured and readable*" -or
                    [string]$json.friend_mp_sync_transport_adapter_reason -notlike "*not implemented yet*") {
                    throw "SNC tray status JSON did not expose configured friend_mp_sync_transport_adapter_reason."
                }
                if ([string]$json.friend_mp_sync_transport_adapter_next_step -notlike "*manual MP package export/import*" -or
                    ...
                    throw "SNC tray status JSON did not expose ready_disabled friend_mp_sync_transport_adapter_next_step."
                }
                ...
                if ($summaryText -notlike "*friend_mp_sync_transport_adapter_state:*" -or
