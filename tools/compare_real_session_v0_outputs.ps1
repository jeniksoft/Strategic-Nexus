...
$previousIntegratedEmpireStatePresent = "false"
$currentIntegratedEmpireStatePresent = "false"
$previousIntegratedEmpireStateParsed = "false"
$currentIntegratedEmpireStateParsed = "false"
$previousIntegratedEmpireStateCampaignKey = ""
$currentIntegratedEmpireStateCampaignKey = ""
$previousIntegratedEmpireStateEmpireName = ""
$currentIntegratedEmpireStateEmpireName = ""
$previousIntegratedEmpireStateGovernment = ""
$currentIntegratedEmpireStateGovernment = ""
$previousIntegratedEmpireStateAuthority = ""
$currentIntegratedEmpireStateAuthority = ""
$previousIntegratedEmpireStateOwnedFleetCount = "0"
$currentIntegratedEmpireStateOwnedFleetCount = "0"
$previousIntegratedEmpireStateActiveWarCount = "0"
$currentIntegratedEmpireStateActiveWarCount = "0"
$previousIntegratedEmpireStateMissingFields = @()
$currentIntegratedEmpireStateMissingFields = @()
$previousIntegratedEmpireStateUncertainties = @()
$currentIntegratedEmpireStateUncertainties = @()
...
$previousPostPlayDoctrineAlignmentNote = ""
$currentPostPlayDoctrineAlignmentNote = ""
...
if (Test-Path -LiteralPath $previousEvidencePath) {
    ...
    if ($null -ne $previousEvidence.entry_point_post_play) {
        ...
        if ($null -ne $previousEvidence.entry_point_post_play.integrated_empire_state) {
            ...
            if ([string]::IsNullOrWhiteSpace($previousIntegratedEmpireStatePresent)) { $previousIntegratedEmpireStatePresent = "false" }
            $previousIntegratedEmpireStateParsed = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "parsed"
            if ([string]::IsNullOrWhiteSpace($previousIntegratedEmpireStateParsed)) { $previousIntegratedEmpireStateParsed = "false" }
            $previousIntegratedEmpireStateCampaignKey = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "campaign_key"
            $previousIntegratedEmpireStateEmpireName = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "empire_name"
            $previousIntegratedEmpireStateGovernment = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "government"
            $previousIntegratedEmpireStateAuthority = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "authority"
            $previousIntegratedEmpireStateOwnedFleetCount = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "owned_fleet_count"
            $previousIntegratedEmpireStateActiveWarCount = Get-OptionalString -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "active_war_count"
            $previousIntegratedEmpireStateMissingFields = Get-OptionalStringArray -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "missing_fields"
            $previousIntegratedEmpireStateUncertainties = Get-OptionalStringArray -Object $previousEvidence.entry_point_post_play.integrated_empire_state -Property "uncertainties"
        }
        if ($null -ne $previousEvidence.entry_point_post_play.personality_profile) {
            ...
            $previousPostPlayDoctrineAlignmentNote = Get-OptionalString -Object $previousEvidence.entry_point_post_play -Property "post_play_doctrine_alignment_note"
...
if (Test-Path -LiteralPath $currentEvidencePath) {
    ...
    if ($null -ne $currentEvidence.entry_point_post_play) {
        ...
        if ($null -ne $currentEvidence.entry_point_post_play.integrated_empire_state) {
            ...
            if ([string]::IsNullOrWhiteSpace($currentIntegratedEmpireStatePresent)) { $currentIntegratedEmpireStatePresent = "false" }
            $currentIntegratedEmpireStateParsed = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "parsed"
            if ([string]::IsNullOrWhiteSpace($currentIntegratedEmpireStateParsed)) { $currentIntegratedEmpireStateParsed = "false" }
            $currentIntegratedEmpireStateCampaignKey = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "campaign_key"
            $currentIntegratedEmpireStateEmpireName = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "empire_name"
            $currentIntegratedEmpireStateGovernment = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "government"
            $currentIntegratedEmpireStateAuthority = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "authority"
            $currentIntegratedEmpireStateOwnedFleetCount = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "owned_fleet_count"
            $currentIntegratedEmpireStateActiveWarCount = Get-OptionalString -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "active_war_count"
            $currentIntegratedEmpireStateMissingFields = Get-OptionalStringArray -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "missing_fields"
            $currentIntegratedEmpireStateUncertainties = Get-OptionalStringArray -Object $currentEvidence.entry_point_post_play.integrated_empire_state -Property "uncertainties"
        }
        if ($null -ne $currentEvidence.entry_point_post_play.personality_profile) {
            ...
            $currentPostPlayDoctrineAlignmentNote = Get-OptionalString -Object $currentEvidence.entry_point_post_play -Property "post_play_doctrine_alignment_note"
...
$result = [ordered]@{
    ...
    integrated_empire_state = [ordered]@{
        present_previous = $previousIntegratedEmpireStatePresent
        present_current = $currentIntegratedEmpireStatePresent
        present_changed = ($previousIntegratedEmpireStatePresent -ne $currentIntegratedEmpireStatePresent)
        parsed_previous = $previousIntegratedEmpireStateParsed
        parsed_current = $currentIntegratedEmpireStateParsed
        parsed_changed = ($previousIntegratedEmpireStateParsed -ne $currentIntegratedEmpireStateParsed)
        campaign_key_previous = $previousIntegratedEmpireStateCampaignKey
        campaign_key_current = $currentIntegratedEmpireStateCampaignKey
        campaign_key_changed = ($previousIntegratedEmpireStateCampaignKey -ne $currentIntegratedEmpireStateCampaignKey)
        empire_name_previous = $previousIntegratedEmpireStateEmpireName
        empire_name_current = $currentIntegratedEmpireStateEmpireName
        empire_name_changed = ($previousIntegratedEmpireStateEmpireName -ne $currentIntegratedEmpireStateEmpireName)
        government_previous = $previousIntegratedEmpireStateGovernment
        government_current = $currentIntegratedEmpireStateGovernment
        government_changed = ($previousIntegratedEmpireStateGovernment -ne $currentIntegratedEmpireStateGovernment)
        authority_previous = $previousIntegratedEmpireStateAuthority
        authority_current = $currentIntegratedEmpireStateAuthority
        authority_changed = ($previousIntegratedEmpireStateAuthority -ne $currentIntegratedEmpireStateAuthority)
        owned_fleet_count_previous = $previousIntegratedEmpireStateOwnedFleetCount
        owned_fleet_count_current = $currentIntegratedEmpireStateOwnedFleetCount
        owned_fleet_count_changed = ($previousIntegratedEmpireStateOwnedFleetCount -ne $currentIntegratedEmpireStateOwnedFleetCount)
        active_war_count_previous = $previousIntegratedEmpireStateActiveWarCount
        active_war_count_current = $currentIntegratedEmpireStateActiveWarCount
        active_war_count_changed = ($previousIntegratedEmpireStateActiveWarCount -ne $currentIntegratedEmpireStateActiveWarCount)
        missing_fields_previous = @($previousIntegratedEmpireStateMissingFields)
        missing_fields_current = @($currentIntegratedEmpireStateMissingFields)
        missing_fields_changed = ((($previousIntegratedEmpireStateMissingFields -join "|") -ne ($currentIntegratedEmpireStateMissingFields -join "|")).ToString().ToLowerInvariant())
        uncertainties_previous = @($previousIntegratedEmpireStateUncertainties)
        uncertainties_current = @($currentIntegratedEmpireStateUncertainties)
        uncertainties_changed = ((($previousIntegratedEmpireStateUncertainties -join "|") -ne ($currentIntegratedEmpireStateUncertainties -join "|")).ToString().ToLowerInvariant())
    }
    post_play_personality_profile = [ordered]@{
        ...
        doctrine_alignment_note_previous = $previousPostPlayDoctrineAlignmentNote
        doctrine_alignment_note_current = $currentPostPlayDoctrineAlignmentNote
        doctrine_alignment_note_changed = ($previousPostPlayDoctrineAlignmentNote -ne $currentPostPlayDoctrineAlignmentNote)
...
Write-Host ("real_session_v0_compare_integrated_empire_state_present_current=" + $currentIntegratedEmpireStatePresent)
Write-Host ("real_session_v0_compare_integrated_empire_state_present_previous=" + $previousIntegratedEmpireStatePresent)
Write-Host ("real_session_v0_compare_integrated_empire_state_present_changed=" + ((($previousIntegratedEmpireStatePresent -ne $currentIntegratedEmpireStatePresent).ToString().ToLowerInvariant())))
Write-Host ("real_session_v0_compare_integrated_empire_state_parsed_current=" + $currentIntegratedEmpireStateParsed)
Write-Host ("real_session_v0_compare_integrated_empire_state_parsed_previous=" + $previousIntegratedEmpireStateParsed)
Write-Host ("real_session_v0_compare_integrated_empire_state_parsed_changed=" + ((($previousIntegratedEmpireStateParsed -ne $currentIntegratedEmpireStateParsed).ToString().ToLowerInvariant())))
Write-Host ("real_session_v0_compare_integrated_empire_state_campaign_key_current=" + $currentIntegratedEmpireStateCampaignKey)
Write-Host ("real_session_v0_compare_integrated_empire_state_campaign_key_previous=" + $previousIntegratedEmpireStateCampaignKey)
Write-Host ("real_session_v0_compare_integrated_empire_state_campaign_key_changed=" + ((($previousIntegratedEmpireStateCampaignKey -ne $currentIntegratedEmpireStateCampaignKey).ToString().ToLowerInvariant())))
Write-Host ("real_session_v0_compare_integrated_empire_state_empire_name_current=" + $currentIntegratedEmpireStateEmpireName)
Write-Host ("real_session_v0_compare_integrated_empire_state_empire_name_previous=" + $previousIntegratedEmpireStateEmpireName)
Write-Host ("real_session_v0_compare_integrated_empire_state_empire_name_changed=" + ((($previousIntegratedEmpireStateEmpireName -ne $currentIntegratedEmpireStateEmpireName).ToString().ToLowerInvariant())))
Write-Host ("real_session_v0_compare_integrated_empire_state_owned_fleet_count_current=" + $currentIntegratedEmpireStateOwnedFleetCount)
Write-Host ("real_session_v0_compare_integrated_empire_state_owned_fleet_count_previous=" + $previousIntegratedEmpireStateOwnedFleetCount)
Write-Host ("real_session_v0_compare_integrated_empire_state_owned_fleet_count_changed=" + ((($previousIntegratedEmpireStateOwnedFleetCount -ne $currentIntegratedEmpireStateOwnedFleetCount).ToString().ToLowerInvariant())))
Write-Host ("real_session_v0_compare_integrated_empire_state_active_war_count_current=" + $currentIntegratedEmpireStateActiveWarCount)
Write-Host ("real_session_v0_compare_integrated_empire_state_active_war_count_previous=" + $previousIntegratedEmpireStateActiveWarCount)
Write-Host ("real_session_v0_compare_integrated_empire_state_active_war_count_changed=" + ((($previousIntegratedEmpireStateActiveWarCount -ne $currentIntegratedEmpireStateActiveWarCount).ToString().ToLowerInvariant())))
...
Write-Host ("real_session_v0_compare_post_play_doctrine_alignment_note_current=" + $currentPostPlayDoctrineAlignmentNote)
Write-Host ("real_session_v0_compare_post_play_doctrine_alignment_note_previous=" + $previousPostPlayDoctrineAlignmentNote)
Write-Host ("real_session_v0_compare_post_play_doctrine_alignment_note_changed=" + ((($previousPostPlayDoctrineAlignmentNote -ne $currentPostPlayDoctrineAlignmentNote).ToString().ToLowerInvariant())))
